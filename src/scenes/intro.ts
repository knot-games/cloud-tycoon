...
export class IntroScene extends BaseScene {
	...
	public create(): void {
		...
		this.nameInput = this.add.dom(getGameWidth(this) / 2 + 100, 500).createFromCache("nameForm").setOrigin(0.5, 0.5);

		this.returnKey = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ENTER);

		this.returnKey.on("down", event => {
			let name: HTMLInputElement = <HTMLInputElement>this.nameInput.getChildByName("name");
