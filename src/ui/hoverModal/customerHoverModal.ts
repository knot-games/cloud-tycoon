...
export const customerHoverModal = (
	scene: Phaser.Scene,
	...
	const title = scene.add
		.text(width / 2 + x, yValue, 'Customers', { fontFamily: 'Arial', fontSize: '20px', color: colorPalette.white })
		.setOrigin(0.5, 0.5);
	...
	for (const [key, value] of Object.entries(levelState.customers)) {
		const serverEntry = scene.add
			.text(x + 20, yValue, value.name + ': ' + customers[Number(key)], {
				fontFamily: 'Arial',
				fontSize: '16px',
