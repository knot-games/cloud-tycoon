...
export const costHoverModal = (
	...
	// Facilities
	const facilityTitle = scene.add
		.text(width / 2 + x, yValue, 'Facility', { fontFamily: 'Arial', fontSize: '20px', color: colorPalette.white })
		.setOrigin(0.5, 0.5);
	costHoverModalItems.push(facilityTitle);
	yValue += 24;

	const currentFacility = levelState.facilities[facility];
	const facilityEntry = scene.add
		.text(x + 20, yValue, currentFacility.name + ': $' + currentFacility.cost + '/month', {
			fontFamily: 'Arial',
			fontSize: '16px',
	...
	// Servers
	const serverTitle = scene.add
		.text(width / 2 + x, yValue, 'Server Costs', {
			fontFamily: 'Arial',
			fontSize: '20px',
			color: colorPalette.white,
		})
	...
	for (const [key, value] of Object.entries(levelState.servers)) {
		const serverEntry = scene.add
			.text(
				x + 20,
				yValue,
				value.name + ': $' + Math.floor(servers[Number(key)] * value.monthlyCost) + '/month',
