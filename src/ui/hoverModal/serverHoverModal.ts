...
export const serverHoverModal = (
	...
	const title = scene.add
		.text(width / 2 + x, yValue, 'Servers', { fontFamily: 'Arial', fontSize: '20px', color: colorPalette.white })
		.setOrigin(0.5, 0.5);
	...
	for (const [key, value] of Object.entries(levelState.servers)) {
		...
		const serverEntry = scene.add
			.text(x + 20, yValue, value.name + ': ' + servers[Number(key)], {
				fontFamily: 'Arial',
				fontSize: '16px',
	...
	const stats = scene.add
		.text(width / 2 + x, yValue, 'Stats', { fontFamily: 'Arial', fontSize: '20px', color: colorPalette.white })
		.setOrigin(0.5, 0.5);
	...
	const capacity = scene.add
		.text(
			x + 20,
			yValue,
			'Capacity: ' + totalCustomers + '/' + totalCapacity + ' (' + remainingCapacity + '% remaining)',
