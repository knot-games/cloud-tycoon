...
export const hudMenu = (scene: Phaser.Scene, game: Game, levelState: Level) => {
    ...
    // Main Menu
    const mainMenuItem = menuItem(scene, 0, getGameHeight(scene) - (1 * menuItemHeight) - 40, menuWidth, menuItemHeight, "Main Menu", "home", () => {
        console.log("Main Menu Clicked!");
    });
    menuItems.push(mainMenuItem);

    // Research
    const researchItem = menuItem(scene, 0, getGameHeight(scene) - (2 * menuItemHeight) - 40, menuWidth, menuItemHeight, "Research", "flask", () => {
        console.log("Research clicked!");
    });
    menuItems.push(researchItem);

    // Employees
    const employeesItem = menuItem(scene, 0, getGameHeight(scene) - (3 * menuItemHeight) - 40, menuWidth, menuItemHeight, "Employees", "person", () => {
        console.log("Employees clicked!");
    });
    menuItems.push(employeesItem);

    // Products
    const productsItems = menuItem(scene, 0, getGameHeight(scene) - (4 * menuItemHeight) - 40, menuWidth, menuItemHeight, "Products", "coin", () => {
...
const menuItem = (scene: Phaser.Scene, x: number, y: number, width: number, height: number, text: string, icon: string, onClick: () => void) => {
    // Add background
    const menuItem = scene.add.rectangle(x, y, width, height, getColorInt(colorPalette.menuBar)).setOrigin(0, 0).setInteractive({ useHandCursor: true }).on('pointerdown', () => {
        onClick();
    });
    const graphics = scene.add.graphics();
    graphics.lineStyle(3, getColorInt(colorPalette.blue), 1)
        .strokeRect(x, y, width - 1, height - 1);
