...
export const storeModal = function (scene: Phaser.Scene, game: Game, levelState: Level, onClose: () => void) {
  ...
  for (const [key, value] of Object.entries(serverOptions)) {
    const item = storeItem(scene, storeItemX, storeItemY, key, value.name, value.description, value.maximum, ownedServers[Number(key)], value.cost, value.monthlyCost, value.capacity, true);
  ...
  for (const [key, value] of Object.entries(storeOptions)) {
    ...
    const item = storeItem(scene, storeItemX, storeItemY, key, value.name, value.description, 1, totalOwned, value.cost, 0, 0, false);
    storeItems.push(item);
    storeItemY += 120;
  }
...
const storeItem = function (scene: Phaser.Scene, x: number, y: number, id: string, title: string, description: string, maximum: number, currentlyOwned: number, cost: number, monthlyCost: number, capacity: number, isServer: boolean) {
  ...
  const itemTitle = scene.add.text(x + 88, y + 20, title, { fontFamily: "Arial", fontSize: "20px", color: colorPalette.white, fontStyle: 'bold' });
  storeItems.push(itemTitle);

  let quantityLabel = quantity(scene, getGameWidth(scene) - 75 - 54, y + 32, currentNumber)
  ...
  const itemCostDetails = scene.add.text(x + 88, y + 64, `Cost: $${cost} Monthly Cost: $${monthlyCost} ${isServer ? `Capacity: ${capacity}` : ''}`, { fontFamily: "Arial", fontSize: "16px", color: colorPalette.white });
...
const plusButton = function (scene: Phaser.Scene, x: number, y: number, onClick: () => void) {
  const button = scene.add.text(x, y, "+")
    ...
    .on('pointerover', () => { button.setStyle({ backgroundColor: colorPalette.burntOrange, color: colorPalette.white }) })
    .on('pointerup', () => { button.setStyle({ backgroundColor: colorPalette.burntOrange, color: colorPalette.white }) })
    .on('pointerout', () => { button.setStyle({ backgroundColor: accentColor, color: colorPalette.white }) });
  return button;
}
...
const quantity = function (scene: Phaser.Scene, x: number, y: number, currentQuantity: number) {
  return scene.add.text(x, y, currentQuantity.toString(), { fontFamily: "Arial", fontSize: "16px", color: colorPalette.white }).setOrigin(0.5, 0.5);
}
...
const cost = function (scene: Phaser.Scene, x: number, y: number) {
  return scene.add.text(x, y, costText(cartState.totalCost), { fontFamily: "Arial", fontSize: "16px", color: colorPalette.white, fontStyle: 'bold' }).setOrigin(0.5, 0.5);
}
...
const setErrorText = function (scene: Phaser.Scene, text: string) {
  ...
  errorText = scene.add.text(getGameWidth(scene) - 225, getGameHeight(scene) - 75, text, { fontFamily: "Arial", fontSize: "16px", color: colorPalette.white, fontStyle: 'bold' }).setOrigin(0.5, 0.5);
}

const purchaseButton = function (scene: Phaser.Scene, game: Game, level: Level, x: number, y: number) {
  const button = scene.add.text(x, y, "Purchase")
    .setOrigin(0.5, 0.5)
    .setPadding(8)
    .setStyle({ fontSize: '16px', color: colorPalette.white, backgroundColor: accentColor, align: 'center' })
    .setInteractive({ useHandCursor: true })
